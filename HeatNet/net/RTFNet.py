...
class RTFNet(nn.Module):
    def __init__(self, n_class, num_resnet_layers):
        ...
        self.deconv5 = self._make_transpose_layer(TransBottleneck, self.n_class, 2, stride=2)
        
        #########################
        self.classifier = Classifier_Module(256, [6, 12, 18, 24], [6, 12, 18, 24], n_class)
    ...
    def forward(self, input):
        ...
        rgb_tmp = rgb
        rgb_tmp = self.classifier(rgb_tmp)
...
# def unit_test():
#     num_minibatch = 2
#     rgb = torch.randn(num_minibatch, 3, 640, 480).cuda(0)
#     thermal = torch.randn(num_minibatch, 1, 640, 480).cuda(0)
#     rtf_net = RTFNet(9,18).cuda(0)
