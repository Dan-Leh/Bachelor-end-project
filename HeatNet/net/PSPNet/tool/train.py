...
from util.util import AverageMeter, poly_learning_rate, intersectionAndUnionGPU, find_free_port, plot_img
...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # print(input.shape, type(target), type(output))
        out_label = output[0].cpu().numpy()
        tar_label = target[0].cpu().numpy()
        ...
        if main_process():
            ...
            out_img, tar_img = plot_img(out_label, tar_label)
            writer.add_image('output', out_img, dataformats='HWC')
            writer.add_image('target', tar_img, dataformats='HWC')
