    ...
    for line in list_read:
        ...
        if split == 'test':
            ...
            if domain == 'day':
                ...
                label_name = os.path.join(data_root, set, domain, 'label', 'gray', 'fl_ir_aligned'+line+'.png').replace('\\', '/')
...
class Freiburg_Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = np.concatenate((rgb,thermal[...,None]), axis=2)
        ...
        if self.split == 'train':
            ...
            if self.domain == 'night':
                image_name = label_path.split('/')[-1].split('.')[0]
                path = '/home/jsun/Project/Freiburg/train/night/cert/' + image_name + '.npy'
                cert = np.load(path)
        else:
