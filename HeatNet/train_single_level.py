...
# from model.HeatNet_RTF import HeatNet_RTF
from util.dataset import Freiburg_Dataset
from util import transform

IMG_MEAN = [0.485, 0.456, 0.406, 0.622]
IMG_STD = [0.229, 0.224, 0.225, 0.171]
...
EPOCHS = 150
POWER = 0.9
RESTORE_FROM = '/home/jsun/Project/HeatNet-master/snapshots/train_epoch_100.pth'
#RESTORE_FROM = ''
WEIGHT = ''
ROOT = '/home/jsun/Project/Freiburg/'
LOG_SAVE_PATH = '/home/jsun/Project/HeatNet-master/logs'
SNAPSHOT_DIR = '/home/jsun/Project/HeatNet-master/snapshots/'
WEIGHT_DECAY = 0.0005
...
def get_arguments():
    ...
    """
    parser = argparse.ArgumentParser(description="HeatNet-PSPNet Network")
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i_iter in range(max_iter):
            ...
            images_source, labels = trainloader_iter.next()

            images_target, _ = targetloader_iter.next()
            
            loss = train(epoch_log, model, model_D1, images_source, labels, images_target, optimizer,\
                  optimizer_D1, source_label, target_label, i_iter+1, current_iter, max_iter, interp, interp_target)
        ...
        if (epoch_log % 1 == 0):
            ...
            torch.save({'epoch': epoch_log, 'model_state_dict': model.state_dict(), \
                        ...
                        'optimizer_D1': optimizer_D1.state_dict()}, filename)
            if epoch_log / 1 > 2:
                deletename = args.snapshot_dir + '/train_epoch_' + str(epoch_log - 1 * 2) + '.pth'
                os.remove(deletename)

def train(epoch_log, model, model_D1, images_source, labels, images_target, optimizer, \
          optimizer_D1, source_label, target_label, i_iter, current_iter, max_iter, interp, interp_target):
    ...
    pred_target1, _ = model(images_target)
    ...
    loss_adv_batch = args.lambda_adv * loss_adv_target1
    ...
    writer.add_scalar('loss_dis_batch',loss_D_value1, current_iter)
    
    return loss_seg_batch, loss_adv_batch, loss_D_value1
