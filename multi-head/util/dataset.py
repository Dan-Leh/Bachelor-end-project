...
import sys
sys.path.append('/home/jsun/Project/semseg-master')
...
# train_val_split('E:/graduation project/Freiburg/train/day/')

def make_dataset(split='train', data_root=None, domain=None, modality=None):
    ...
    for line in list_read:
        ...
        if split == 'test':
            if modality == 'rgb':
                image_name = os.path.join(data_root, set, domain, 'RGB', line+'.png').replace('\\', '/')
            else:
                image_name = os.path.join(data_root, set, domain, 'thermal', line[:-5]+'_ir.png').replace('\\', '/')
            label_name = os.path.join(data_root, set, domain, 'label', line+'.png').replace('\\', '/')
        else:
            if modality == 'rgb':
                image_name = os.path.join(data_root, set, domain, 'RGB', 'fl_rgb'+line+'.png').replace('\\', '/')
            else:
                image_name = os.path.join(data_root, set, domain, 'thermal', 'fl_ir_aligned'+line+'.png').replace('\\', '/')
...
class Freiburg_Dataset(data.Dataset):
    def __init__(self, split='train', data_root=None, domain=None, modality=None, transform=None):
        self.split = split
        self.domain = domain
        self.data_list = make_dataset(split, data_root, domain, modality)
